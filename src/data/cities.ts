...
// Function to get city data by city slug only
export async function getCityBySlugFromDB(citySlug: string): Promise<CityDetail | null> {
  try {
    const { data, error } = await supabase
      .from('cities')
      .select('*')
    ...
    console.error('Error in getCityBySlugFromDB:', error);
    return null;
  }
}
...
// Function to check if a city is available by city slug only
export async function isCityAvailableBySlugFromDB(citySlug: string): Promise<boolean> {
  try {
    const { count, error } = await supabase
      .from('cities')
      .select('*', { count: 'exact', head: true })
    ...
    console.error('Error in isCityAvailableBySlugFromDB:', error);
    return false;
  }
}
...
// Function to generate metadata for a city page with just city slug
export async function generateCityMetadataBySlug(citySlug: string) {
  try {
    // Get SEO data directly from database
    const { data, error } = await supabase
      .from('cities')
